"""
Generate synthetic UL Solutions industrial equipment certification PDFs.
Creates realistic documents with GD&T specs, material tables, test results,
and equipment diagrams for AI document parsing demos.
"""
...
COMPLIANCE_STANDARDS = ["UL", "CSA", "IEC 61439", "NEMA 250", "IEEE C37", "NEC Article 409", "EN 60204-1", "ISO 13849-1"]
...
def create_equipment_diagram(width: float = 400, height: float = 200, eq_type: str = "") -> Drawing:
    """Create a simplified equipment block diagram."""
    d = Drawing(width, height)
...
def generate_equipment_pdf(doc_index: int) -> dict:
    """Generate a single equipment certification PDF and return metadata."""
    ...
    story.append(Paragraph(
        ...
        "Third-angle projection is used throughout. Tolerances follow the geometric "
        "dimensioning and tolerancing (GD&T) specifications detailed in Section 3.",
        body_style
    ))
    ...
    story.append(Paragraph(
        ...
        "housing and mounting interfaces per ASME Y14.5-2018. All measurements were verified "
        "using coordinate measuring machine (CMM) inspection.",
        body_style
    ))
    ...
    story.append(Paragraph(
        ...
        f"applicable {standards} standards. Testing was performed at an accredited "
        f"UL Solutions laboratory under controlled environmental conditions.",
        body_style
    ))
    ...
    for test_name in selected_tests:
        if "Dielectric" in test_name:
            measured = f"{random.randint(1800, 2500)} VAC"
            threshold = "≥ 1500 VAC"
        elif "Insulation" in test_name:
            measured = f"{random.randint(80, 500)} MΩ"
            threshold = "≥ 50 MΩ"
        elif "Ground" in test_name:
            measured = f"{round(random.uniform(0.01, 0.15), 3)} Ω"
            threshold = "≤ 0.1 Ω"
...
def main():
    ...
    print(f"Generating {NUM_DOCS} industrial equipment certification PDFs...")
    ...
    print(f"\nDone! {NUM_DOCS} PDFs saved to: {OUTPUT_DIR}")
    print("\nGenerated documents:")
    for d in docs:
        print(f"  • {d['certification_id']} | {d['model_number']} | {d['manufacturer']} | Status: {d['certification_status']}")
